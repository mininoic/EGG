...
(function(){
    ...
    var addMouseTouchEvent= function(_this){
        ...
        var checkMouse = function(e){
            ...
            if(ismouse)checkInteraction(e.pageX, e.pageY);
        }
        $(window).click(checkMouse).on({ 'touchstart' : checkTouch });
        var checkInteraction = function(x, y){ 
            ...
            // console.log(canvas.style.marginLeft,x,y)
            var e={x:x,y:y};
            _this._ismouseDown=true;
            var length=_this.childrentList.length - 1;
            for (var i = length; i >= 0; i--) {
                // console.log(BKGM.checkEventActor( e,_this.childrentList[i]))
